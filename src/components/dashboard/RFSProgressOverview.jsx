...
export default function RFSProgressOverview() {
  ...
  // Chart 1: Completion Distribution (Overall)
  const completedCount = processedData.filter(o => o.status === 'Completed').length;
  ...
  // Chart 3: Fiber vs NaaS Share (Completed Sites Only)
  const completedOrders = processedData.filter(o => o.status === 'Completed');
  const fiberCompleted = completedOrders.filter(o => o.service_type === 'fiber' || !o.service_type).length;
  const naasCompleted = completedOrders.filter(o => o.service_type === 'naas').length;

  const typeShareData = [
    { name: 'Fiber', value: fiberCompleted },
    { name: 'NaaS', value: naasCompleted }
  ];

  // Chart 2: Monthly Progress
  // Group completed sites by month (YYYY-MM)
  const monthlyGroups = {};
  
  completedOrders.forEach(order => {
    // Use delivery_conf_date or updated_date as completion date proxy
    const dateStr = order.delivery_conf_date || order.updated_date;
    if (!dateStr) return;
